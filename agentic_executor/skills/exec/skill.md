...
name: exec
description: |
  Do NOT wait for the user to ask for /exec. Use this skill on your own initiative whenever
  a task would require two or more tool calls. Reading two files? One call. Editing multiple
  lines across multiple files? One call. Finding files then modifying them? Two calls
  This skill gives you a local Python interpreter with execute() always in scope — full standard
  library, loops, conditionals, chained results. Use it for: batch file operations, multi-step
  workflows, codebase scanning and analysis, running CLI tools and processing their output,
  any find-then-modify or search-then-edit pattern, easy, harnessing python power.
allowed-tools:
  - Bash
---
